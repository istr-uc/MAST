      ...
      for I in 1..Max_Transactions loop
         ...
         if Unbounded_Time or Analysis_Was_Aborted then
            ...
            loop
               ...
               -- mar 2025: Previously the deadline*stop_factor was 
               -- compared to the length of the busy period here,
               -- to determine if analysis should be aborted.
               -- This comparison has been moved forward, to
               -- compare with actual response times, not busy period lenghts
            ...
            if Unbounded_Time or Analysis_Was_Aborted then
               ...
               for Job in 1..NumJobs loop
                  ...
                  for J in 2..Ni loop
                     ...
                     -- Mar 2025: Moved from calculation of busy period
                     if Stop_Factor_When_Not_Schedulable/=Positive'Last and then
                       Rij+Transaction(I).The_Task(1).Jinit>
                       Time(1*Stop_Factor_When_Not_Schedulable)*Di
                     then
                        Analysis_Was_Aborted:=True;
                        exit;
                     end if;
                  ...
                  exit when Analysis_Was_Aborted;
               end loop;
            end if;
         end if;
