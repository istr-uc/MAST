...
package body Mast.Monoprocessor_Tools is
   ...
   begin
      ...
      for I in 1..Max_Transactions loop
         ...
         if Verbose then
            if Preemptible then
               Put_Line("-- PREEMPTIBLE TASK --");
            else
               Put_Line("-- NON-PREEMPTIBLE TASK --");
            end if;
         ...
         if not Preemptible then
            ...
            while Tant/=Tm loop
               ...
               for J in 1..Max_Transactions loop
                  exit when Transaction(J).Ni=0;
                  if Transaction(I).The_Task(Ni).Prioij<=Transaction(J).The_Task(Transaction(J).Ni).Prioij then --Higher or equal priority
                     Tm := Tm + Ceiling((Tant+Transaction(J).The_Task(Transaction(J).Ni).Jinit)/Transaction(J).The_Task(Transaction(J).Ni).Tijown)*Transaction(J).The_Task(Transaction(J).Ni).Cijown;
                  end if;
               end loop;
            end loop;
            Qm := Ceiling((Tm+Ji)/Ti);
            ...
            for K in 0..Integer(Qm)-1 loop
               ...
               while Want/=Wm loop
                  ...
                  for J in 1..Max_Transactions loop
                     exit when Transaction(J).Ni=0;
                     if Transaction(I).The_Task(Ni).Prioij<=Transaction(J).The_Task(Transaction(J).Ni).Prioij and Transaction_ID'Image(Transaction(I).Transaction_Id)/=Transaction_ID'Image(Transaction(J).Transaction_Id) then --(Higher priority) UPDATE: Changed to higher or equal priority, but exclude the actual task from the pool
                        --Wm := Wm + Ceiling((Want+Transaction(J).The_Task(Transaction(J).Ni).Jinit)/Transaction(J).The_Task(Transaction(J).Ni).Tijown)*Transaction(J).The_Task(Transaction(J).Ni).Cijown;
                        Wm := Wm + (Floor((Want+Transaction(J).The_Task(Transaction(J).Ni).Jinit)/Transaction(J).The_Task(Transaction(J).Ni).Tijown)+1.0)*Transaction(J).The_Task(Transaction(J).Ni).Cijown;  --Floor+1 instead of Ceiling
                     end if;
                  end loop;
               end loop;
